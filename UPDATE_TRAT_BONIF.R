...
CLI AS (SELECT CLICODIGO, GCLCODIGO GRUPO  FROM CLIEN
         WHERE CLICLIENTE='S'),
...
SELECT CLICODIGO, GRUPO,T.TBPCODIGO COD_TABELA, 
RIGHT(TBPDESCRICAO,1) TIPO,TBPDTINICIO INICIO,TBPDTVALIDADE VALIDADE, 
TBPSITUACAO SITUACAO ,TBPPCOVENDA TOTAL_ACORDO
FROM TABPRECO T
...
trat_t_qtd <- dbGetQuery(con2,"
  ...
  CLI AS (SELECT CLICODIGO,CLINOMEFANT,GCLCODIGO FROM CLIEN WHERE CLICODIGO= :CLIENTE),
  
  PED AS (SELECT ID_PEDIDO,P.CLICODIGO,CLINOMEFANT,GCLCODIGO,PEDDTEMIS
  FROM PEDID P
  INNER JOIN FIS ON P.FISCODIGO1=FIS.FISCODIGO
  INNER JOIN CLI ON P.CLICODIGO=CLI.CLICODIGO
  WHERE PEDDTEMIS 
   BETWEEN (SELECT T.TBPDTINICIO FROM TABPRECO T
        INNER JOIN CLITBP C ON C.TBPCODIGO=T.TBPCODIGO 
          WHERE T.TBPCODIGO=:TBCODIGO AND CLICODIGO=:CLIENTE)
          AND 'TODAY' AND PEDSITPED<>'C'),
  ...
  PROD AS (SELECT DISTINCT PROCODIGO,TBPPRODU.TBPCODIGO FROM TBPPRODU
  ...
  PED_PROMO_PLUGIN AS (SELECT ID_PEDIDPROMOCAO ID_PEDIDO_PROMO FROM PEDIDPROMO P2
  ...
  SELECT CLICODIGO,CLINOMEFANT,GCLCODIGO GRUPO,P3.ID_PEDIDO,PEDDTEMIS,
  P3.PROCODIGO,PDPQTDADE QTD,
  PDPUNITLIQUIDO VRVENDA
  FROM PDPRD P3
  INNER JOIN PED ON P3.ID_PEDIDO=PED.ID_PEDIDO
  INNER JOIN PROD ON P3.PROCODIGO=PROD.PROCODIGO
  LEFT OUTER JOIN PED_PROMO_UNION ON P3.ID_PEDIDO=PED_PROMO_UNION.ID_PEDIDO_PROMO
  WHERE ID_PEDIDO_PROMO IS NULL 
  GROUP BY 1,2,3,4,5,6,7,8 HAVING PDPUNITLIQUIDO<=1 ORDER BY ID_PEDIDO DESC
  
  INTO :CLICODIGO,:NOME,:GRUPO, :ID_PEDIDO,:EMISSAO, :PROCODIGO,:QTD,:VRVENDA
